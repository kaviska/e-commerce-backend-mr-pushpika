...
class CartController extends Controller
{
    ...
    public function index(Request $request)
    {
        $validated = $request->validate([
            'user_id' => 'integer',
            'guest_cart' => 'array',
            'guest_cart.*.stock_id' => 'required|integer|exists:stocks,id',
        ...
        if ($request->has('guest_cart')) {
            ...
            $response['cart_items'] = $this->getCartItems($userId, $request->with ?? 'all', $request->limit ?? $this->DEFAULT_LIMIT, $request->offset ?? $this->DEFAULT_OFFSET);
        }
        ...
        if (!$request->has('guest_cart')) {
            ...
            if ($request->has('type')) {
                return "kk";
                $response['total_amount'] = $this->getTotalCartAmount($response['cart_items'], false, $request->type ?? PlatformType::POS->value);
                $response['total_with_discounts'] = $this->getTotalCartAmount($response['cart_items'], $request->with_discount ?? true, $request->type ?? PlatformType::POS->value);
    ...
    public function getCartItems(int $userId, string $with = "all", int $limit = 10, int $offset = 0)
    {
        ...
        $withQueryMap = [
            ...
            "all" => [
                ...
                "stock.product.taxonomies",
                "stock.variationStocks.variationOption.variation"
            ],
        ];
        ...
        $cartItems = $cartItems->limit(10000)->offset(0)->get();
    ...
    public function getTotalCartAmount($cartItems, $withDiscount = false, $type = PlatformType::WEB->value, $isGuestCart = false)
    {
        ...
        foreach ($cartItems as $cartItem) {
            ...
            if ($isGuestCart) {
                $itemTotalAmount = $cartItem["stock"][$priceKey] * $cartItem["quantity"];
            } else {
                $itemTotalAmount = $cartItem->stock->$priceKey * $cartItem->quantity;
            }
            ...
            if ($withDiscount) {
                if ($isGuestCart) {
                    $itemTotalAmount -= $cartItem["stock"][$discountKey] * $cartItem["quantity"];
                } else {
                    $itemTotalAmount -= $cartItem->stock->$discountKey * $cartItem->quantity;
                }
    ...
    public function translation($cartItems)
    {
        $validator = validator()->make($cartItems, [
            ...
            'cart_items.*.stock_id' => 'required|integer|exists:stocks,id',
